...
function IO_Config()
    ...
    temp = GPIOSETFUNC(HAPTIC_PWM,DIGITAL_OUT,FREQ)     //HAPTICS WILL USE FREQ NOT PWM OUTPUT...only one PWM timer
    //To set haptics frequency: GPIOWRITE(HAPTIC_PWM, 0 to N) max frequency a few kHz likely. Try N=1000. Set N=0 to end haptics.
    ...
    temp = GPIOSETFUNC(DEBUG_LED,DIGITAL_OUT,000111)     //OUTPUT DRIVE STRENGTH bits 4-6 = 111, bits 0-3 = 000
